  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      <div className="flex flex-col items-center text-center">
        <div
          className={`text-3xl font-bold ${getScoreColor(languageExpression)}`}
        >
          {languageExpression}
        </div>
      ...
      <div className="flex flex-col items-center text-center">
        <div
          className={`text-3xl font-bold ${getScoreColor(emotionalManagement)}`}
        >
          {emotionalManagement}
        </div>
      ...
      <div className="flex flex-col items-center text-center">
        <div
          className={`text-3xl font-bold ${getScoreColor(logicalStructure)}`}
        >
          {logicalStructure}
        </div>
      ...
      <div className="flex flex-col items-center text-center">
        <div
          className={`text-3xl font-bold ${getScoreColor(communicationEffectiveness)}`}
        >
          {communicationEffectiveness}
        </div>
    ...
    <ul className="space-y-4">
      {suggestions.map((suggestion, index) => (
        <li key={index} className="flex items-start gap-3">
          <div
            className={`p-2 rounded-full mt-0.5 ${theme === "dark" ? "bg-blue-900/30" : "bg-blue-100"}`}
          >
...
const MessageAnalysisPanel: React.FC<{
  ...
  const radarData = [
    ...
    {
      name: "沟通效果",
      value: analysis.communicationEffectiveness,
      color: "#ef4444",
    },
    {
      name: "信息完整",
      value: analysis.informationCompleteness,
      color: "#06b6d4",
    },
  ];
  ...
  return (
    <div className={theme === "dark" ? "text-gray-200" : "text-gray-800"}>
      {analysis.feedback && (
        <div
          className={`mb-6 p-4 rounded-lg border ${theme === "dark" ? "bg-blue-900/20 border-blue-800 text-blue-300" : "bg-blue-50 border-blue-100 text-blue-800"}`}
        >
      ...
      {analysis.communicationStyle && (
        <div
          className={`mb-6 p-4 rounded-lg border ${theme === "dark" ? "bg-purple-900/20 border-purple-800 text-purple-300" : "bg-purple-50 border-purple-100 text-purple-800"}`}
        >
      ...
      {analysis.strengths && analysis.strengths.length > 0 && (
        <div
          className={`mb-6 p-4 rounded-lg border ${theme === "dark" ? "bg-green-900/20 border-green-800 text-green-300" : "bg-green-50 border-green-100 text-green-800"}`}
        >
      ...
      {analysis.suggestions && analysis.suggestions.length > 0 && (
        <div
          className={`mb-6 p-4 rounded-lg border ${theme === "dark" ? "bg-yellow-900/20 border-yellow-800 text-yellow-300" : "bg-yellow-50 border-yellow-100 text-yellow-800"}`}
        >
...
const SuggestionCard: React.FC<{
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-center">
        <div
          className={`text-xs px-2 py-1 rounded-full ${theme === "dark" ? "bg-blue-900/30 text-blue-300" : "bg-blue-100 text-blue-700"}`}
        >
...
export default function AIPracticePage() {
  ...
  const scenarios: Scenario[] = [
    {
      ...
      title: "项目汇报",
      description:
        "向你的经理汇报项目进展，包括已完成工作、遇到的挑战和下一步计划。",
      category: "工作场景",
      difficulty: "medium",
    ...
    try {
      ...
      const response = await client.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "你是一位专业的沟通顾问，擅长分析和评价沟通质量。",
          },
  ...
  const generateImprovementSuggestions = (
    ...
    if (analysis.languageExpression < 3) {
      suggestions.push(
        "建议使用更清晰、简洁的语言表达，避免冗长和模糊的表述。"
      );
    }

    if (analysis.emotionalManagement < 3) {
      suggestions.push(
        "可以尝试更恰当地表达情绪，使用积极的语气增强沟通效果。"
      );
    }

    if (analysis.logicalStructure < 3) {
      suggestions.push(
        "建议先明确核心观点，再有条理地展开论述，使用结构化的表达方式。"
      );
    }

    if (analysis.empathy < 3) {
      suggestions.push(
        "尝试从对方的角度思考问题，表达理解和支持，增强共情能力。"
      );
    }
    ...
    if (suggestions.length === 0) {
      suggestions.push(
        "继续保持良好的沟通风格，尝试在适当的场合使用更丰富的表达方式。"
      );
  ...
  const loadConversationFromHistory = (conversation: Conversation) => {
    ...
    setSelectedScenario(
      scenarios.find((s) => s.id === conversation.scenarioId) || null
    );
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const analysis: MessageAnalysis = {
        ...
        emotionalManagement:
          aiAnalysis.emotionalManagement || Math.floor(Math.random() * 3) + 3,
        logicalStructure:
          aiAnalysis.logicalStructure || Math.floor(Math.random() * 3) + 3,
        communicationEffectiveness:
          aiAnalysis.communicationEffectiveness ||
        ...
        improvementSuggestions:
          ...
          generateImprovementSuggestions({
            languageExpression:
              aiAnalysis.languageExpression ||
              Math.floor(Math.random() * 3) + 3,
            emotionalManagement:
              aiAnalysis.emotionalManagement ||
              Math.floor(Math.random() * 3) + 3,
            logicalStructure:
              aiAnalysis.logicalStructure || Math.floor(Math.random() * 3) + 3,
            communicationEffectiveness:
      ...
      if (isMountedRef.current) {
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === userMessage.id ? { ...msg, analysis } : msg
          )
        );
      ...
      if (isMountedRef.current && aiAnalysis.suggestedResponses) {
        ...
        setSuggestedResponses(
          generateSuggestedResponses(newMessage, selectedScenario)
        );
      }
        ...
        messages: [
          {
            role: "system",
            content:
              "你是一位专业的沟通伙伴，擅长根据不同场景和对话历史生成自然、合适的响应。",
          },
  ...
  const analyzeConversation = async (
    ...
    try {
      ...
      const conversationText = messages
        .map(
          (msg) => `${msg.sender === "user" ? "用户" : "AI"}: ${msg.content}`
        )
        .join("\n");
      ...
      const response = await client.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "你是一位专业的沟通顾问，擅长分析完整对话的沟通质量。",
          },
          { role: "user", content: prompt },
        ],
        temperature: 0.3,
  ...
  const savePracticeRecord = (conversationAnalysis: any) => {
    ...
    const score = conversationAnalysis?.overallScore
      ? Math.round(conversationAnalysis.overallScore * 20)
    ...
    // 保存练习记录到本地存储
    const practiceRecords = JSON.parse(
      localStorage.getItem("practice_records") ||
        JSON.stringify(mockPracticeRecords)
    );
  ...
  // 获取总体分析
  const { score: overallScore, feedback: overallFeedback } =
    calculateOverallAnalysis(messages);
                      ...
                      <div className="flex justify-start mb-6">
                        ...
                        >
                          <div className="flex space-x-1">
                            ...
                            <div
                              className="animate-bounce w-2 h-2 rounded-full bg-gray-400"
            ...
            >
              <div className="mb-6">
                ...
                >
                  ...
                  >
                    练习时长:{" "}
                    {Math.round((Date.now() - conversationStartTime) / 60000)}{" "}
                    分钟
                  </p>
                </div>
              </div>
